...
export function DotPattern({
  ...
  dotSize = 2,
  gap = 24,
  // --border (resting dots — barely visible against --background)
  baseColor = "#2c2825",
  // --primary (glow on mouse proximity — matches the app's accent)
  glowColor = "#e69c3a",
  proximity = 120,
  glowIntensity = 1,
  waveSpeed = 0.5,
  ...
  return (
    <div ref={containerRef} className={cn("fixed inset-0 overflow-hidden bg-background", className)}>
      ...
      {/* Content layer */}
      {children && <div className="relative z-10 flex h-full w-full flex-col">{children}</div>}
    </div>
  )
}
