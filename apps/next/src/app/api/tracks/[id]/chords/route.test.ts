...
function makeRequest(cookie?: string) {
  return new NextRequest(new URL("/api/tracks/track-1/chords", "http://localhost"), {
...
describe("GET /api/tracks/[id]/chords", () => {
  beforeEach(() => {
  ...
  it("forwards the token and returns the chord segments", async () => {
    const chords = [{ id: "seg-1", startTime: 0, endTime: 2.5, root: "C", chordType: "maj" }];
    ...
    expect(fetch).toHaveBeenCalledWith("http://nest-test:3001/tracks/track-1/chords", {
      headers: { Authorization: "Bearer signed-jwt" },
    });
    expect(response.status).toBe(200);
    expect(await response.json()).toEqual(chords);
  });
